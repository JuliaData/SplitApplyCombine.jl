...
function leftgroupjoin2(lkey, rkey, f, ::typeof(isequal), left::AbstractArray, right::AbstractArray)
    # isequal heralds a hash-based approach, roughly O(length(left) * log(length(right)))
    T = promote_op(f, eltype(left), eltype(right))
    K = promote_op(rkey, eltype(right))
    V = eltype(right)
    ...
    g = groupinds(rkey, right)

    out = Dict{K2, Vector{T}}()
    @inbounds for a ∈ left
        key = lkey(a)
        group = get!(() -> T[], out, key)
        g_index = Base.ht_keyindex(g, key)
        if g_index > 0 # -1 if key not found
            for ri ∈ g.vals[g_index]
                push!(group, f(a, right[ri]))
            end
        end
    end
    return out
